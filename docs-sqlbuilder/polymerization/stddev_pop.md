...
public class Test {
    ...
    public void function() {
        ...
        Function function = Function.getFunction(FunctionType.STDDEV_POP, age);
...
````sql
SELECT STDDEV_POP(t."age") FROM "test" t     
````
...
### CLICKHOUSE
````sql
SELECT stddevPop(t.age) FROM test t     
````
...
````sql
SELECT stdevp(t.age) FROM test t     
````
