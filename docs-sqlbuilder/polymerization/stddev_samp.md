...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.STDDEV_SAMP, num);
...
````sql
SELECT STDDEV_SAMP(t."num") AS "val" FROM "test" t 
````
...
### CLICKHOUSE
````sql
SELECT IF (isNaN(stddevSamp(t.num)) == 1 or  isInfinite(stddevSamp(t.num)) ==1 ,NULL , stddevSamp(t.num)) AS val FROM test t 
````
...
````sql
SELECT VARP(t.num) AS val FROM test t
````
