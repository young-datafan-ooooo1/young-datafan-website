...
public class Test {
    ...
    public void function() {
        ...
        Function function = Function.getFunction(FunctionType.STANDARD_ERROR, age);
...
````sql
SELECT STDDEV_SAMP(t."age")/SQRT(count(1)) FROM "test" t   
````
...
### CLICKHOUSE
````sql
SELECT if(COUNT(1) = 0, null, stddevSamp(t.age)/SQRT(count(1))) FROM test t     
````
...
````sql
SELECT STDEV(t.age)/SQRT(count(1)) FROM test t     
````
