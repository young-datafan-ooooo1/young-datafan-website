...
public class Test {
    ...
    public void function() {
        ...
        Function function = Function.getFunction(FunctionType.COVAR_POP, age, salary);
...
````sql
SELECT covar_pop(t."age",t."salary") FROM "test" t     
````

### MYSQL
````sql
SELECT (VAR_POP(t.`age`+t.`salary`) - VAR_POP(t.`age`) - VAR_POP(t.`salary`))/2 FROM `test` t     
````
...
### CLICKHOUSE
````sql
SELECT covarPop(t.age,t.salary) FROM test t     
````
...
````sql
SELECT (varp(t.age+t.salary) - varp(t.age) - varp(t.salary))/2 FROM test t     
````
