...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.VAR_SAMP, num);
...
````sql
SELECT VAR_SAMP(t."num") AS "val" FROM "test" t 
````
...
### CLICKHOUSE
````sql
SELECT IF (isNaN(varSamp(t.num)) == 1 or  isInfinite(varSamp(t.num)) ==1 ,NULL , varSamp(t.num)) AS val FROM test t 
````
