...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.MONTH_BEGIN, date);
...
````sql
SELECT trunc(t."date", 'MM' ) AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT DATE_ADD( LAST_DAY( date_sub(t.`date`, INTERVAL 1 MONTH )), INTERVAL 1 DAY ) AS `val` FROM `test` t 
````

### POSTGRESQL
````sql
SELECT cast(date_trunc('month', t.date) as date) AS val FROM test t   
````

### CLICKHOUSE
````sql
SELECT toStartOfMonth(t.date) AS val FROM test t  
````
...
````sql
SELECT CONVERT(date, CONVERT ( VARCHAR ( 8 ), t.date, 23 ) + '01' ) AS val FROM test t 
````