...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.QUARTER_END, date);
...
````sql
SELECT add_months( trunc(t."date", 'Q' ), 3 )-1 AS "val" FROM "test" t    
````

### MYSQL

````sql
SELECT DATE_ADD( STR_TO_DATE( concat( date_format(t.`date`, '%Y' ), '-01-31' ), '%Y-%m-%d' ), INTERVAL (3 * EXTRACT( QUARTER FROM t.`date`)-1) MONTH ) AS `val` FROM `test` t     
````

### POSTGRESQL
````sql
SELECT cast(((date_trunc('quarter', t.date) + INTERVAL '3 month' ) - INTERVAL '1 day') as date) AS val FROM test t  
````

### CLICKHOUSE
````sql
SELECT subtractDays(addMonths(toStartOfQuarter(t.date),3),1) AS val FROM test t  
````
...
````sql
SELECT CONVERT(date,DATEADD(quarter, 1, DATEADD( quarter, DATEDIFF( quarter, 0, t.date), - 1))) AS val FROM test t   
````