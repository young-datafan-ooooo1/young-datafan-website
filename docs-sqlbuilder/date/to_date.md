...
````sql
SELECT TO_DATE(t."date",'yyyy-MM-dd') AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT STR_TO_DATE(t.`date`,'%Y-%m-%d') AS `val` FROM `test` t  
````
...
### CLICKHOUSE
````sql
SELECT toDate(t.date) AS val FROM test t 
````
...
````sql
SELECT CONVERT(date,t.date,23) AS val FROM test t 
````