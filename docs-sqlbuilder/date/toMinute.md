...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.MINUTE, date);
...
````sql
SELECT TO_NUMBER(TO_CHAR(t."date", 'MI')) AS "val" FROM "test" t 
````

### MYSQL

````sql
SELECT MINUTE(t.`date`) AS `val` FROM `test` t
````

### POSTGRESQL
````sql
SELECT EXTRACT(MINUTE from t.date) AS val FROM test t 
````

### CLICKHOUSE
````sql
SELECT toMinute(toDateTime(t.date)) AS val FROM test t 
````
...
````sql
SELECT DATEPART(Minute, t.date) AS val FROM test t
````