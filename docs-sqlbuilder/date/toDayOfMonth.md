...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.DAY, date);
...
````sql
SELECT TO_NUMBER(TO_CHAR(t."date",'dd')) AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT DAY(t.`date`) AS `val` FROM `test` t
````

### POSTGRESQL
````sql
SELECT EXTRACT(DAY FROM t.date) AS val FROM test t  
````

### CLICKHOUSE
````sql
SELECT toDayOfMonth(toDate(t.date)) AS val FROM test t
````
