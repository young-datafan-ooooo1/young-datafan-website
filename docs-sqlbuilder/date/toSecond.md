...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.SECOND, date);
...
````sql
SELECT TO_NUMBER(TO_CHAR(t."date", 'SS')) AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT SECOND(t.`date`) AS `val` FROM `test` t 
````

### POSTGRESQL
````sql
SELECT EXTRACT(SECOND from t.date) AS val FROM test t  
````

### CLICKHOUSE
````sql
SELECT toSecond(toDateTime(t.date)) AS val FROM test t 
````
...
````sql
SELECT DATEPART(Second, t.date) AS val FROM test t
````