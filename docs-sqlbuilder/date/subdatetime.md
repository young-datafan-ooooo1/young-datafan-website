...
public class Test {
    ...
    public void getFunction() {
        ...
        CustomSql num = CustomSql.getCustomSql("1");
        Query query = new Query();
        Function function = Function.getFunction(FunctionType.SUB_DATE_TIME, date, num, timeUtil);
...
````sql
SELECT add_months(t."date",-1*1*12) AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT date_sub(t.`date`, interval 1 year) AS `val` FROM `test` t   
````

### POSTGRESQL
````sql
SELECT (t.date+INTERVAL '-1*1 year') AS val FROM test t  
````

### CLICKHOUSE
````sql
SELECT subtractYears(t.date,1) AS val FROM test t
````
...
````sql
SELECT DATEADD(year,-1*1, t.date) AS val FROM test t 
````