...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.QUARTER_BEGIN, date);
...
````sql
SELECT trunc(t."date", 'Q' ) AS "val" FROM "test" t 
````

### MYSQL

````sql
SELECT DATE_ADD( STR_TO_DATE( concat( date_format(t.`date`, '%Y' ), '-01-01' ), '%Y-%m-%d' ), INTERVAL 3 * (EXTRACT( QUARTER FROM t.`date`)-1) MONTH ) AS `val` FROM `test` t     
````

### POSTGRESQL
````sql
SELECT cast(date_trunc( 'quarter', t.date) as date) AS val FROM test t   
````

### CLICKHOUSE
````sql
SELECT toStartOfQuarter(t.date) AS val FROM test t     
````
...
````sql
SELECT CONVERT(date,DATEADD( quarter, 1, DATEADD( quarter, DATEDIFF( quarter, 0,t.date) - 1,0))) AS val FROM test t  
````