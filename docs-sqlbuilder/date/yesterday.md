...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.YESTERDAY);
...
````sql
SELECT TRUNC(SYSDATE - 1) AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) AS `val` FROM `test` t   
````

### POSTGRESQL
````sql
SELECT TO_DATE(TO_CHAR(CURRENT_DATE + INTERVAL '-1 DAY','YYYY-MM-DD'),'YYYY-MM-DD') AS val FROM test t    
````

### CLICKHOUSE
````sql
SELECT yesterday() AS val FROM test t  
````
...
````sql
SELECT CONVERT ( VARCHAR ( 10 ), DATEADD( DAY,- 1, GETDATE( ) ), 120 ) AS val FROM test t 
````