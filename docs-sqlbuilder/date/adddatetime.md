...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.ADD_DATE_TIME, date, num, timeUtil);
...
````sql
SELECT add_months(t."date",1*12) AS "val" FROM "test" t   
````

### MYSQL

````sql
SELECT date_add(t.`date`, interval 1 year) AS `val` FROM `test` t  
````

### POSTGRESQL
````sql
SELECT (t.date+INTERVAL '1 year') AS val FROM test t  
````

### CLICKHOUSE
````sql
SELECT addYears(t.date,1) AS val FROM test t     
````
...
````sql
SELECT DATEADD(year,1, t.date) AS val FROM test t  
````