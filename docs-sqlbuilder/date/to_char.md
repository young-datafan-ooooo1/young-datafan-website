...
````sql
SELECT TO_CHAR(t."date",'yyyy-MM-dd') AS "val" FROM "test" t 
````

### MYSQL

````sql
SELECT DATE_FORMAT(t.`date`,'%Y-%m-%d') AS `val` FROM `test` t 
````
...
### CLICKHOUSE
````sql
SELECT formatDateTime(t.date,'%Y-%m-%d') AS val FROM test t 
````
...
````sql
SELECT CONVERT(varchar(100),t.date,'23') AS val FROM test t 
````