...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.CURRENT_DATE);
...
````sql
SELECT SYSDATE AS "val" FROM "test" t   
````

### MYSQL

````sql
SELECT CURRENT_DATE() AS `val` FROM `test` t  
````
...
### CLICKHOUSE
````sql
SELECT formatDateTime(now(),'%Y-%m-%d') AS val FROM test t    
````
...
````sql
SELECT CONVERT(VARCHAR(10) ,GETDATE(), 120) AS val FROM test t    
````