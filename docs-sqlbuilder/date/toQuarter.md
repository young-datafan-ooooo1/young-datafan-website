...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.QUARTER, date);
...
````sql
SELECT TO_CHAR(t."date", 'Q') AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT QUARTER(t.`date`) AS `val` FROM `test` t 
````

### POSTGRESQL
````sql
SELECT EXTRACT(QUARTER FROM t.date) AS val FROM test t  
````

### CLICKHOUSE
````sql
SELECT toQuarter(toDate(t.date)) AS val FROM test t  
````
...
````sql
SELECT DATEPART(QUARTER,t.date) AS val FROM test t
````