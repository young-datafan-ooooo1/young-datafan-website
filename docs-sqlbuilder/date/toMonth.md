...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.MONTH, date);
...
````sql
SELECT EXTRACT(MONTH FROM t."date") AS "val" FROM "test" t   
````

### MYSQL

````sql
SELECT MONTH(t.`date`) AS `val` FROM `test` t
````
...
### CLICKHOUSE
````sql
SELECT toMonth(toDate(t.date)) AS val FROM test t  
````
