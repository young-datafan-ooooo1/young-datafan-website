...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.NUMBER_CHAR, num);
...
````sql
SELECT  TO_CHAR(t."num")  AS "val" FROM "test" t  
````

### MYSQL

````sql
SELECT CAST(t.`num` as CHAR) AS `val` FROM `test` t 
````

### POSTGRESQL
````sql
SELECT CAST(t.num AS VARCHAR) AS val FROM test t   
````

### CLICKHOUSE
````sql
SELECT toString(t.num) AS val FROM test t   
````
