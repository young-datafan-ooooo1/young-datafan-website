...
````sql
SELECT TRUNC(t."num",0) AS "val" FROM "test" t   
````

### MYSQL

````sql
SELECT CONVERT(t.`num`, UNSIGNED INTEGER) AS `val` FROM `test` t  
````

### POSTGRESQL
````sql
SELECT toDecimal64(t.num, 0) AS val FROM test t 
````

### CLICKHOUSE
````sql
SELECT ROUND(t.num, 2) AS val FROM test t    
````
...
````sql
SELECT CAST(t.num AS int) AS val FROM test t
````