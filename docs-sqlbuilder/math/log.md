...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.LOG, num);
...
````sql
SELECT LOG(EXP(1),t."num") AS "val" FROM "test" t   
````

### MYSQL

````sql
SELECT LOG(t.`num`) AS `val` FROM `test` t
````

### POSTGRESQL
````sql
SELECT LOG(2.718281828459045,t.num) AS val FROM test t 
````
