...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.LOG10, num);
...
````sql
SELECT LOG(10, t."num") AS "val" FROM "test" t 
````
...
### CLICKHOUSE
````sql
SELECT LOG10(t.num) AS val FROM test t  
````
...
````sql
SELECT LOG(t.num, 10) AS val FROM test t  
````