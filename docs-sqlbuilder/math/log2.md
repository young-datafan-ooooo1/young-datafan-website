...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.LOG2, num);
...
````sql
SELECT LOG(2, t."num") AS "val" FROM "test" t  
````
...
### CLICKHOUSE
````sql
SELECT LOG2(t.num) AS val FROM test t 
````
...
````sql
SELECT LOG(t.num, 2) AS val FROM test t 
````