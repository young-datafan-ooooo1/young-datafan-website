...
public class Test {
    ...
    public void getFunction() {
        ...
        Function function = Function.getFunction(FunctionType.CBRT, num);
...
````sql
SELECT POWER(t."num", 1/3) AS "val" FROM "test" t 
````

### MYSQL

````sql
SELECT POW(t.`num`, 1.0/3.0) AS `val` FROM `test` t  
````

### POSTGRESQL
````sql
SELECT CBRT(t.num) AS val FROM test t  
````
